...
| `regression-harness.md` | Test layer model (L0–L4), regression contract (8-point DoD), story wiring metadata schema, critical path, build-test loop. | 5 — reference |
| `PROGRESS.md` | Auto-updated story tracker (Story 11.4). Contributor reads this at task-startup. | — runtime |
...
**Companion (outside `backlog/`):** `CONTRIBUTING.md` — dev environment setup guide (authoritative for what's installed, what's missing, who downloads what).
...
- **Post-pass-4 deltas:** T-31 generalized (reference hardware no longer pinned to a specific Intel SKU; any modern 8+ core CPU with per-machine calibration). T-43 added (`cargo-llvm-cov`, replacing `cargo-tarpaulin` which is Linux-only). T-44 added (dev-env prerequisites as a threshold). Story 0.7 added (env.ps1 + verify-dev-env.ps1 + fetch_ohm.ps1). Story 3.2 annotated with no-NVIDIA-hardware caveat. PRD §11 + architecture §12 added (Development Environment sections). Tarpaulin→llvm-cov fix applied to Story 11.2, Story 10.1.
...
## Ingestion protocol (updated pass 4)
...
- **Pass 4 (regression harness & story wiring):** New Epic 11 (4 stories). New `regression-harness.md` (layer model L0–L4, 8-point DoD, wiring schema, critical path, build-test loop). Every story now carries a `Wiring:` block. New guardrails G25 (cumulative regression — full matrix per PR), G26 (coverage non-regression — T-42 floor), G27 (wiring discipline — `Depends-On`/`Blocks`/`Next` are the source of truth). New thresholds T-40 (per-layer budgets), T-41 (aggregate ≤750s), T-42 (coverage delta ≥0%). New fixture F14 (regression triple-layer test). HITL matrix expanded: +2 story gates (11.1, 11.4), +6 action gates (layer model, regression contract, snapshot acceptance, progress schema, coverage target).
- **Post-pass-4 (dev environment inventory):** Documented the relocatable `tools/` folder structure, the activation script (`scripts/env.ps1`), and the who-downloads-what split (system prereqs = user; project tooling = contributor via cargo-binstall + scoop; signing = CI-only). New Story 0.7 (env.ps1 + verify-dev-env.ps1 + fetch_ohm.ps1). **Corrected `cargo-tarpaulin` → `cargo-llvm-cov`** everywhere (Story 11.2, Story 10.1) — tarpaulin is Linux-only. Generalized T-31 reference hardware (now any modern 8+ core CPU with per-machine calibration). New T-43 (coverage tool), T-44 (dev-env prerequisites). PRD §11 + architecture §12 added. Story 3.2 annotated: NVML integration tests are `#[ignore]`'d without NVIDIA hardware; AMD GPU coverage via Story 3.6 (OHM) only.
