...
- Full workspace matrix (`cargo test --workspace --all-features --all-targets
  --target x86_64-pc-windows-msvc`): **663 passed, 0 failed, 13 ignored**.
...
| 15.1 | partial | 2026-07-16T00Z | epic-15-batch2 | — | sidebar-monitor-host.exe: C#5-compatible source, compiles + verified on HW. BUT the host is NOT wired into the app (OhmSupervisor still uses HTTP path). Story is partial until 15.2 wiring reaches production. |
| 15.2 | partial | 2026-07-16T00Z | epic-15-batch2 | L0:2 | SensorSource trait + PipeSource impl + HttpSource bridge. 2 tests. BUT trait is NOT used by production code (provider_registry still constructs OhmAdapter with HTTP). |
| 15.3 | deferred | 2026-07-16T00Z | — | — | Delete dead HTTP code: deferred until pipe host is wired into production (15.1/15.2 must reach merged first). |
| 16.1 | partial | 2026-07-16T00Z | epic-16-batch3 | — | sidebar-monitor-svc.exe: service binary compiles, registered, started, pipe created. BUT the app never connects to the service pipe; the service spawns a host whose data goes nowhere. Partial until 16.2 wires the IPC client. |
| 16.2 | deferred | 2026-07-16T00Z | — | — | Pipe IPC client protocol: the Rust-side named-pipe consumer (PipeServiceSource) is not coded. Connection verified but no production wiring. |
...
| 17.3 | merged | 2026-07-16T00Z | epic-17-remaining | — | Bandwidth CSV export: Export button writes escaped luid,adapter_name,rx_bytes,tx_bytes rows and shows persistent success/failure feedback. |
...
- Merged: 77 / 96 (80.2%) — Stories 0.1-0.7, 1.1-1.6, 2.1-2.3, 3.1-3.6, 4.1-4.3, 5.1-5.3, 6.1-6.5, 7.1-7.5, 8.1-8.10, 10.1, 11.1, 11.3, 11.4, 12.1-12.3, 12.6, 12.8, 13.1-13.5, 14.1-14.4, 16.3-16.4, 16.6, 17.1-17.7 + INT
- Partial: 8 — 6.6, 9.1, 9.2, 10.2, 11.2, 15.1, 15.2, 16.1
- Deferred: 8 — 9.3, 12.4, 12.5, 12.7, 14.5, 15.3, 16.2, 16.5
- Pending: 1 — 3.2b
...
- Workspace checks recorded for this snapshot: 664 passing, 0 failing, 13 ignored (hardware/UAC/capture smokes). `cargo fmt --all -- --check` and `git diff --check` pass; clippy, deny, Windows-target, release-build, and manual Win11 checks remain separate evidence gates.
- Blocked on HITL: 0
- Long-term blocked: 0
